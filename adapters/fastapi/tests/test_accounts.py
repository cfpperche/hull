...
def test_signup_signin_org_isolation(client, unique: str) -> None:
    a = client.post(
        "/v1/auth/signup",
        json={"username": f"u{unique}a", "email": f"a{unique}@hull.test", "password": "demodemo1"},
    )
    ...
    other = client.post(
        "/v1/auth/signup",
        json={"username": f"u{unique}b", "email": f"b{unique}@hull.test", "password": "demodemo1"},
    )
...
def test_signup_duplicate_email(client, unique: str) -> None:
    body = {"username": f"x{unique}", "email": f"dup{unique}@hull.test", "password": "demodemo1"}
    assert client.post("/v1/auth/signup", json=body).status_code == 201
    again = client.post(
        "/v1/auth/signup",
        json={"username": f"y{unique}", "email": f"dup{unique}@hull.test", "password": "demodemo1"},
    )
...
def test_admin_forbidden(client, unique: str) -> None:
    client.post(
        "/v1/auth/signup",
        json={"username": f"z{unique}", "email": f"z{unique}@hull.test", "password": "demodemo1"},
    )
